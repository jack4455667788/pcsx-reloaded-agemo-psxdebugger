...
u8 psxMemRead8(u32 mem) {
	...
	if (t == 0x1f80 || t == 0x9f80 || t == 0xbf80) {
		if ((mem & 0xffff) < 0x400) {
			tValue = psxHu8(mem);
			Agemo_OnReadMem(mem, 1, tValue);
			return tValue;
		}
		else {
			tValue = psxHwRead8(mem);
			Agemo_OnReadMem(mem, 1, tValue);
			return tValue;
		}
	} else {
...
u16 psxMemRead16(u32 mem) {
	...
	if (t == 0x1f80 || t == 0x9f80 || t == 0xbf80) {
		if ((mem & 0xffff) < 0x400) {
			tValue = psxHu16(mem);
			Agemo_OnReadMem(mem, 2, tValue);
			return tValue;
		}
		else {
			tValue = psxHwRead16(mem);
			Agemo_OnReadMem(mem, 2, tValue);
			return tValue;
		}
	} else {
		...
		if (p != NULL) {
			...
			tValue = SWAPu16(*(u16 *)(p + (mem & 0xffff)));
			Agemo_OnReadMem(mem, 2, tValue);
			return tValue;
		} else {
#ifdef PSXMEM_LOG
...
u32 psxMemRead32(u32 mem) {
	...
	if (t == 0x1f80 || t == 0x9f80 || t == 0xbf80) {
		if ((mem & 0xffff) < 0x400) {
			tValue = psxHu32(mem);
			Agemo_OnReadMem(mem, 4, tValue);
			return tValue;
		}
		else {
			tValue = psxHwRead32(mem);
			Agemo_OnReadMem(mem, 4, tValue);
			return tValue;
		}
	} else {
		...
		if (p != NULL) {
			...
			tValue = SWAPu32(*(u32 *)(p + (mem & 0xffff)));
			Agemo_OnReadMem(mem, 4, tValue);
			return tValue;
		} else {
#ifdef PSXMEM_LOG
