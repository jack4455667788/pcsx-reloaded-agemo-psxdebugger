...
void psxHwWrite32(u32 add, u32 value) {
	switch (add) {
		...
		case 0x1f801810:
			...
			{
				_agemo_gpu_last_status = 1;	//Flush cache
				return;
			}

			if (_agemo_gpu_last_status == 1 && value == 0xA0000000)
			{
				_agemo_gpu_last_status = 2;	//uploading
				return;
			}
			...
			if (_agemo_gpu_last_status == 2)
			{
				...
				_agemo_gpu_last_status = 3;	//end destination X and Y
				return;
			}
			...
			if (_agemo_gpu_last_status == 3)
			{
				...
				_agemo_gpu_last_status = 0;	//Send destination W and H
				return;
			}
			return;
		case 0x1f801814:
